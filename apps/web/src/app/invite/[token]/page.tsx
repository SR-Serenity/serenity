    ...
    async function processInvite() {
      try {
        ...
        if (isAuthResponse(response)) {
          ...
          router.replace(`/${response.organization.slug}`)
        } else {
          ...
          if (auth.isAuthenticated && auth.user && auth.user.email === inviteEmail) {
            // User is logged in with matching email - but backend says needs registration?
            // This shouldn't happen in normal flow, but handle gracefully by redirecting
            setError('Your account is already registered. Please proceed to the workspace.')
            setLoading(false)
